...
async function seed() {
  ...
  const ticketWithSeats = await createTicketWitSeats();
...
async function createTicketWitSeats() {
  const ticketWithSeats = await prisma.ticket.create({
    data: {
      screeningId: 4,
      customerId: 4,
      seat: {
        create: [{ seatNumber: "12" }, { seatNumber: "14" }],
      },
    },
  });
  return ticketWithSeats;
}
